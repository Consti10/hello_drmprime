/*
 * ADDED functionality to use this for latency testing via external LED
 * This effectively measures the hw composer delay.
...
static int modeset_create_fb(int fd, struct modeset_dev *dev)
{
    ...
    ret = drmModeAddFB2WithModifiers(fd, dev->width, dev->height,DRM_FORMAT_XRGB8888,bo_handles,pitches,offsets,modifiers,&dev->fb,0);
	if (ret) {
